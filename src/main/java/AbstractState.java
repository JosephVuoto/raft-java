...
public abstract class AbstractState {
	...
	/* timeout for for the follower to start a new election, this timeout
	 * should be
	 * random to prevent live lock. we define a upper bound and lower bound
	 * here  */
	...
	/* latest term server has seen (initialized to 0 on first boot, increases
	   monotonically) updated on stable storage before responding to RPCs */
	protected static int currentTerm = 0;
	/* candidateId that received vote in current term (or -1 if none). updated
	...
	public AbstractState(NodeImpl node) {
		...
		electionTimeout = ELECTION_TIME_OUT_MIN +
						  new Random().nextInt(ELECTION_TIME_OUT_MAX -
											   ELECTION_TIME_OUT_MIN);
	}
	 ...
	 */
	abstract public int requestVote(int term, int candidateId,
	 ...
	 * @return if the node accepts the request, return 0. If rejects the
	 *     request,
	 * returns its currentTerm, for leader to update itself
	 */
	abstract public int appendEntries(int term, int leaderId, int prevLogIndex,
