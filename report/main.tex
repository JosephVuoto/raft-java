...
so, will discover that depending on the model being considered, no algorithm
can guarantee consensus under all circumstances.
...
tasks in \S\ref{sec:relevant-problems}. We then focus on two popular algorithms
by which to achieve consensus---\textit{Paxos} and \textit{Raft}---detailed in
\S\ref{sec:paxos} and \S\ref{sec:raft}, respectively. A detailed comparative
analysis between the two is provided in \S\ref{sec:analysis}, discussing the
benefits and drawbacks of either algorithm. Finally, we discuss our own
implementation of the \textbf{(Paxos/Raft)} algorithm as part of a
\textbf{(application type)} application, providing a case study of the algorithm
